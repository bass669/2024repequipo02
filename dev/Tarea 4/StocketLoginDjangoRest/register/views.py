...
def signup(request):
    ...
    if email not in emails:
        if password == password1:
            ...
            return render(request, 'register.html', {'loc':report_loc,'errorclass':'alert alert-danger','error': 'Lo siento. Las contraseñas no coinciden.'})
    else:
        print('The Username or Email ID is already taken, returning HTTP response')
        return render(request, 'register.html', {'loc':report_loc,'errorclass':'alert alert-danger','error': 'Lo siento. El nombre de usuario o ID de correo electrónico ya está en uso.'})